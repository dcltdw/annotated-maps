...
- **Visibility-group access control (post-rebuild):** Nodes and notes can be tagged with one or more visibility groups. Read endpoints filter results through a recursive CTE (`backend/src/VisibilityAuth.h`) that walks the parent chain to compute effective visibility — entities are visible only to users in at least one tagged group, with override / inherit semantics. Map owners can opt into `owner_xray` to bypass the filter on maps they own. Cross-organization users cannot be added to a tenant's visibility groups (`org_id` check at member-add time, per #98). Tenant admin or visibility-group manager required to mutate group membership; setting `managesVisibility=true` is admin-only (so managers can't bootstrap themselves into more power).
- **Coordinate-system validation:** `MapController::createMap` and `updateMap` validate the `coordinate_system` JSON as a discriminated union (`wgs84` / `pixel` / `blank`); `image_url` for pixel maps is required to be `http`/`https`. Unknown `type` values are rejected at the controller layer, so the backend stores no malformed shapes and the frontend renderer always has a known type to dispatch on.
...
- **Resource limits:** 1,000 maps/tenant (atomic `INSERT ... SELECT WHERE COUNT < 1000` so concurrent creates can't race past the cap). Per-map node-tree depth capped at `MAX_NODE_DEPTH = 15`, enforced on create / move / copy via a recursive-CTE depth check; cycles rejected on move. Subtree fetches are cursor-paginated so list payloads stay bounded even on wide trees.
- **Audit logging:** `audit_log` records auth events (login success/failure, registration, SSO login, SSO identity collision), membership/permission changes (member add/remove, permission_change, branding update), map mutations (map_update / map_delete), node mutations (node_create / node_update / node_delete / node_move / node_copy / node_visibility_set), note mutations (note_update / note_delete / note_visibility_set), and plot mutations (plot_create / plot_update / plot_delete / plot_node_add / plot_node_remove / plot_note_add / plot_note_remove). Inserts are fire-and-forget with atomic success/failure counters for monitoring.
...
- **`ON DELETE CASCADE` on map owner**: Deleting a user cascades their maps (and therefore all nodes, notes, media, plots-membership, visibility-tagging rows, and permissions). This matches the v0.1 model of "maps are owned by one user." If the product later supports org-owned maps, this should change.
